...
func (ob *Observer) fetchSleep() {
	...
	} else if ob.Executor.GetChainName() == common.ChainMTS {
		time.Sleep(time.Duration(ob.Config.ChainConfig.MTSObserverFetchInterval) * time.Second)
	}
...
func (ob *Observer) fetchBlock(curHeight, nextHeight int64, curBlockHash string) error {
	...
	if curHeight != 0 && parentHash != curBlockHash {
		...
		err = ob.UpdateSwapClaimedConfirmedNum(nextBlockLog.Height)
		if err != nil {
			return err
		}
...
func (ob *Observer) UpdateSwapClaimedConfirmedNum(height int64) error {
	err := ob.DB.Model(model.SwapClaimTxLog{}).Where("chain = ? and status = ?", ob.Executor.GetChainName(), model.TxStatusInit).Updates(
		map[string]interface{}{
			"confirmed_num": gorm.Expr("? - height", height+1),
		}).Error
	if err != nil {
	...
	err = ob.DB.Model(model.SwapClaimTxLog{}).Where("chain = ? and status = ? and confirmed_num >= ?",
		ob.Executor.GetChainName(), model.TxStatusInit, ob.ConfirmNum).Updates(
		map[string]interface{}{
			"status": model.TxStatusConfirmed,
		}).Error
