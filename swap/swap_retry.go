...
//	"time"
//
//	//sabi "github.com/TotemFi/totem-bridge-offchain/abi"
...
//	}
//
//	if retrySwap.Direction == SwapMTS2BSC {
//		bscClientMutex.Lock()
//		defer bscClientMutex.Unlock()
//		data, err := abiEncodeFillMETIS2BSCSwap(ethcom.HexToHash(retrySwap.StartTxHash), swapPairInstance.ERC20Addr, ethcom.HexToAddress(retrySwap.Sponsor), amount, engine.bscSwapAgentABI)
//		if err != nil {
//			return nil, err
//		}
...
//		data, err := abiEncodeFillBSC2ETHSwap(ethcom.HexToHash(retrySwap.StartTxHash), swapPairInstance.ERC20Addr, ethcom.HexToAddress(retrySwap.Sponsor), amount, engine.metisSwapAgentABI)
//		signedTx, err := buildSignedTransaction(engine.metisSwapAgent, engine.metisClient, data, engine.metisPrivateKey)
//		if err != nil {
//			return nil, err
//		}
//		retrySwapTx := &model.RetrySwapTx{
...
//			engine.db.Where("status = ? and track_retry_counter >= ?", model.FillRetryTxSent, engine.config.ChainConfig.MTSMaxTrackRetry).
...
//				if retrySwapTx.Direction == SwapMTS2BSC {
//					chainName = "BSC"
...
//				if retrySwapTx.Direction == SwapBSC2MTS {
...
//	tokenABI, err := abi.JSON(strings.NewReader(sabi.IERC20QueryMetaData.ABI))
//	if err != nil {
//		return "", err
//	}
...
//	if chain == common.ChainMTS {
//		privateKey = engine.metisPrivateKey
//		client = engine.metisClient
//		explorerUrl = engine.config.ChainConfig.MTSExplorerUrl
//		metisClientMutex.Lock()
//		defer metisClientMutex.Unlock()
//	} else {
//		bscClientMutex.Lock()
